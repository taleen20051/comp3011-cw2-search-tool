...
# Configuration values for crawling behaviour.
...
def extract_page_text(soup):
    # Extract all quote text from a parsed HTML page.
    ...
    # Combine all quotes into one searchable string.
    return " ".join(quotes)
...
def crawl_website():
    # Crawl all pages and return extracted page data.
    crawled_pages = []
    visited_urls = set()
    current_url = BASE_URL

    # Continue until no next page exists or a page repeats.
    while current_url and current_url not in visited_urls:
        ...
        if next_url:
            # Respect required 6-second politeness delay.
